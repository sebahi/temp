...
@Controller
public class ProcessInstanceStatusController {
	
	private Logger logger = Logger.getLogger(ProcessInstanceStatusController.class);
	...
	public ModelAndView getAllProcessesDefinitions(Model model) {
		...
		ModelAndView mav = new ModelAndView("processInstanceStatus");
		try {
			...
			for (String string : list) {
				...
				processInstanceList.add(new ProcessInstance(null, null, processInstance.getStatus(), processInstance.getProcessInstanceName(),
						processInstance.getInitiator(), processInstance.getStartDate()));
			}
		...
		mav.addObject("process", processInstanceList);
		return mav;
	}
}
